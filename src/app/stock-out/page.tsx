                                                        ...
                                                        <TableCell className="py-2 !overflow-visible">
                                                            <div className="relative group">
                                                                <Input 
                                                                    ...
                                                                    className="h-8 pr-8 focus:ring-2 focus:ring-red-500/20"
                                                                    ...
                                                                    {productsList.filter(p => !itemSearch[i] || p.name.toLowerCase().includes(itemSearch[i].toLowerCase()) || p.sku.toLowerCase().includes(itemSearch[i].toLowerCase()))
                                                                        ...
                                                                        .map(p => {
                                                                            ...
                                                                            return (
                                                                                <div 
                                                                                    key={p.id} 
                                                                                    className="px-4 py-2.5 hover:bg-red-50 cursor-pointer flex justify-between items-center border-b border-slate-50 last:border-0 transition-colors"
                                                                                    onMouseDown={() => {
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <div className="text-right">
                                                                                        ...
                                                                                        <Badge variant="outline" className="text-xs py-0 h-5 px-1.5 font-bold text-red-600 bg-red-50 border-red-200">{inv?.quantity}</Badge>
                                                                                    </div>
                                                                                </div>
                                                                            )
                                                                        })}
                                                                    {warehouseInventory.length === 0 && (
                                                                        <div className="p-4 text-xs text-slate-400 text-center italic">กรุณาเลือกคลังที่มีสินค้า</div>
                                                                    )}
                                                                </div>
                                                            </div>
                                                        </TableCell>
                                                                ...
                                                                {availSns.length === 0 ? (
                                                                    ...
                                                                    <div className="space-y-3">
                                                                            ...
                                                                            {Array.from({ length: item.quantity }).map((_, snIdx) => (
                                                                                <div key={snIdx} className="relative group">
                                                                                    <Input 
                                                                                        ...
                                                                                        className={`text-sm h-10 font-mono pl-8 shadow-sm transition-all ${item.selected_sns[snIdx] ? "bg-emerald-50 border-emerald-300 text-emerald-700" : "bg-white border-slate-200 focus:border-violet-400 focus:ring-violet-500/10"}`}
                                                                                        value={item.selected_sns[snIdx] || ""}
                                                                                        onChange={(e) => {
                                                                                            const val = e.target.value.trim()
                                                                                            const newSelected = [...item.selected_sns]
                                                                                            // If typed/scanned value exists in available list and is not already picked in another slot
                                                                                            if (availSns.includes(val) && (!newSelected.includes(val) || newSelected[snIdx] === val)) {
                                                                                                newSelected[snIdx] = val
                                                                                                setItems(items.map((x, idx) => idx === i ? { ...x, selected_sns: newSelected } : x))
                                                                                            } else {
                                                                                                // Allow clearing or typing
                                                                                                newSelected[snIdx] = val
                                                                                                setItems(items.map((x, idx) => idx === i ? { ...x, selected_sns: newSelected } : x))
                                                                                            }
                                                                                        }}
                                                                        ...
                                                                        <div className="flex flex-col gap-2 pt-2 border-t border-violet-100">
                                                                            <span className="text-[10px] font-bold text-slate-400 uppercase">Available S/N to pick:</span>
                                                                                ...
                                                                                {availSns.map(sn => {
                                                                                    ...
                                                                                    return (
                                                                                        <button
                                                                                            ...
                                                                                            type="button"
                                                                                            onClick={() => {
                                                                                                // Find first empty slot or toggle off
                                                                                                if (selectedInAnySlot) {
                                                                                                    const newSelected = item.selected_sns.filter(s => s !== sn)
                                                                                                    setItems(items.map((x, idx) => idx === i ? { ...x, selected_sns: newSelected } : x))
                                                                                                } else if (item.selected_sns.length < item.quantity) {
                                                                                                    // Add to end if slots are not all shown or find first empty slot
                                                                                                    const newSelected = [...item.selected_sns]
                                                                                                    const firstEmpty = newSelected.findIndex(s => !s)
                                                                                                    if (firstEmpty !== -1) newSelected[firstEmpty] = sn
                                                                                                    else newSelected.push(sn)
                                                                                                    setItems(items.map((x, idx) => idx === i ? { ...x, selected_sns: newSelected.slice(0, item.quantity) } : x))
                                                                                                }
                                                                                            }}
